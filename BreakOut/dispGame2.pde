void dispGame2() {//ゲーム画面表示用の関数
  frameRate(60+20*combo);//コンボ数によって速さが違う
  if (y < 0) {//ボールが画面上部に達したら跳ね返す
    dy = -dy;
    ddy = dy;
  ...
  if (mouseX+25 >= width) {//マウスの座標が画面外に出た時にはラケットは画面内にとどまる
    mx = width-50;
  } else if (mouseX-25<0) {
    mx = 0;
  } else {
  ...
  if (miscount2 <= 0) {//ゲームオーバー画面へ分岐
    fill1 = 0;
    fill2 = 0;
    GameStatus = "StatusOver";
  }

  if (hit == 90) {//ゲームクリア画面へ分岐
    fill1 = 0;
    fill2 = 0;
    GameStatus = "StatusClear2";
  }
  ...
  for (int i = 0; i<item.length; i=i+2) {//スコアアップ用のアイテムを表示する
    int refItem=0;
    if (item[i] > 0) {//アイテムの配列の中身が0以上の時にアイテムを表示
      pushMatrix();
      translate(i*40+20, 20);
      rotate(frameCount / 300.0);
      image(star, 0, 0, 40, 40);
      popMatrix();
      refItem = checkHitBlock2(i, 0, x, y);//アイテムの当たり判定には大きいブロックの判定を利用
      switch(refItem) {
      case 1:
      case 3:
        player1.trigger();//アイテムにボールが当たった時の音
        item[i]=item[i]-1;//当たったアイテムの部分の配列の中身を1減らす
        itempoint = itempoint + 1;//取ったアイテムの個数のカウントを増やす
        break;
      case 2:
      case 4:
        player1.trigger();//アイテムにボールが当たった時の音
  ...
  if (checkHitCandy(130, 200, 20) || checkHitCandy(270, 200, 20)) {//下の段のキャンディーとの当たり判定
    dx = random(-4, 4);
    ddx = dx;
    if (y<=220) {
      dy = random(-8, -2);
  ...
  for (int i=0; i<hit8.length; i=i+1) {//大きいブロック
    int ref8=0;
    if (hit8[i] > 0) {
      show_block2(i, 120);
      ref8 = checkHitBlock2(i, 3, x, y);
      switch(ref8) {
      case 1:
      case 3:
        player1.trigger();//ブロックにボールが当たった時の音
        hit8[i]=hit8[i]-1;//当たったブロックの部分の配列の中身を1減らす
        hit = hit + 1;//ブロックに当たった回数のカウントを1増やす
        dy = -1*dy;
        break;
      case 2:
  ...
  text(miscount2, 110, height-75);
  text(combo, 110, height-60); 
  text(hit, 110, height-45); 
  text(count*10+combo*100+hit*50+itempoint*100, 110, height-30);//ゲーム中に表示されるスコアはカウントとコンボ数に依存している
  ...
  if (y+ah >= 500 && y+ah <= 505) {//ラケットとの当たり判定
    ...
    ddy = dy;
    if (miscount2>0) {
      point2[4-miscount2]=count*10+combo*100;//ボールを落とした時に、配列内にスコアを保存
      miscount2 = miscount2 -1;//残機数を減らす
    }
    if (miscount2==0) {//残機が0になったら
      point2[0]=count*10+combo*100;
      for (int i=0; i<point2.length; i++) {
        pointsum = pointsum + point2[i];//配列に保存されたスコアを合計する
      }
      pointsum = pointsum + hit * 50 + itempoint * 100;//最終スコアは壊したブロックの数や取ったアイテムの個数も影響してくる
    }
